...
public class PlayerController : MonoBehaviourPunCallbacks, IDamageable, IOnEventCallback
{
    ...
    private void Jump()
    {
        if (Input.GetKeyDown(KeyCode.Space) && (_onFloor || _jumpcount != 0))
        {
    ...
    public void TakeDamage(float damage, Player shooter)
    {
         ...
         */
        Debug.Log("Player: " + _photonView.Owner.NickName + " took " + damage + " damage");
    ...
    void RPC_TakeDamage(float damage, Player shooter)
    {
        ...
        if (_photonView.IsMine)
        {
            Debug.Log("Ja " + _photonView.Owner.NickName + " sam primio " + damage + " damage");
